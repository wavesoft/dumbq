...
($PRINT_JSON = undef) unless (defined ($PRINT_JSON) and ($PRINT_JSON =~ /--print-json/));

# Calculate buffer positions
my $IN_OFS=0; my $IN_SIZE=$FLOPPY_SIZE/2-1;

# Try to open file for input
open FD, "<$FLOPPY" or die "The floppy device (/dev/fdo) can not be opened. Please check the floppy configuration in the Virtual Machine:\n$!";
binmode FD;
...
my ($buf, $data, $n, $dataFull);
while (($n = read FD, $data, 1) != 0) {
    if ($data eq "\0") {
        last;
    }
    if (!defined($PRINT_JSON)) {
...
# See if we need to produce JSON output
if ($PRINT_JSON) {
    print "{\n";
    my $comma = '';
